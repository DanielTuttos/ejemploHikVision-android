...
public class PlayerActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private static final int PERMISSION_REQUEST_READ_FOLDERS = 2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnCapture = findViewById(R.id.btn_captura);
        btnCapture.setOnClickListener(captureJpeg());
        ...
        imagen = findViewById(R.id.imagenGuardada);
    ...
    private View.OnClickListener captureJpeg (){
    ...
    private void capJPEG(){

        File sJpegPicFileName;
        sJpegPicFileName = new File(this.getFilesDir(), "test.jpg") ;

        NET_DVR_JPEGPARA netJpgFile = new NET_DVR_JPEGPARA();
        netJpgFile.wPicQuality = 0;
        netJpgFile.wPicSize = 0xff;

        // boolean result = netSDKInstance.NET_DVR_CaptureJPEGPicture(loginId, startChannel, netJpgFile, sJpegPicFileName);
        if(!netSDKInstance.NET_DVR_CaptureJPEGPicture(loginId, startChannel, netJpgFile, String.valueOf(sJpegPicFileName))){
            Log.e(TAG, "Fallo la captura"+ String.valueOf(sJpegPicFileName));
            Toast.makeText(this, "Fallo la captura " + netSDKInstance.NET_DVR_GetLastError(), Toast.LENGTH_SHORT).show();
            return;
        }else{
          Log.i(TAG, "Captura realizada correctamente");
            Toast.makeText(this, "Captura realizada correctamente", Toast.LENGTH_SHORT).show();

            File imgFile = new File(String.valueOf(sJpegPicFileName));
